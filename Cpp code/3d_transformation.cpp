...
int main() {
    ...
    // Original 3D Bar coordinates (x1, y1, x2, y2, depth, topflag)
    ...
    // Original 3D Bar (Red Color)
    setcolor(RED);
    bar3d(x1, y1, x2, y2, depth, 1);

    // Transformed 3D Bar Color (WHITE)
    setcolor(WHITE);
    ...
    else if (choice == 3) { // 3D Rotation (around Z-axis)
        float angle = 30;
